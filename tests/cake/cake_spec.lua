require("plenary.reload").reload_module("cake", true)
local cake = require("cake")
local default = cake.config
...
describe("tests", function()
  it("works with default values", function()
    cake.setup()
    assert.are.same(cake.config, default)
  end)
  ...
  it("works with config overrides", function()
    ...
    cake.setup({ undercurl = false, underline = false })
    assert.are.same(cake.config, expected)
  end)
  ...
  it("does not set terminal colors when terminal_colors is false", function()
    clear_term_colors()
    cake.setup({ terminal_colors = false })
    cake.load()
  ...
  it("sets terminal colors when terminal_colors is true", function()
    clear_term_colors()
    cake.setup({ terminal_colors = true })
    cake.load()

    -- dark bg
    local colors = require("cake").palette
